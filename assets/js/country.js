...
function loadLineChart(json, jsonDelta, dataType) {
  ...
  let datesFormated = dates.map((dateJson) => {
  ...
  let avg = _.times(values.length, _.constant(_.mean(values).toFixed(0)));

  linesChart = new Chart(document.getElementById("linhas"), {
    type: "line",
    data: {
      labels: datesFormated,
      datasets: [
        {
          data: values,
...
function loadKPI(json) {
  ...
  document.getElementById("kpirecovered").innerText = (_.last(json).Confirmed - _.last(json).Deaths).toLocaleString("PT");
}
